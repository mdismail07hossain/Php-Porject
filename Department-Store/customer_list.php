  ...
  <!-- display category list  -->
   ...
   <div class="container my-5">
        ...
        <h2 class="">Customer List</h2>
            <div>
            <a href="add_coustomer.php" class="btn btn-success d-block my-2" role="button">
        Add Customer
        </a>
            </div>
        </div>
        <table class="table table-striped table-bordered align-middle">
            <thead class="table-success">
                <tr>
                    ...
                    <th scope="col">Identifier Mobile</th>
            ...
            <?php
                    $active_supplier='';
                    $supplier_list = $db->query("SELECT * FROM customer");
                    if($supplier_list->num_rows > 0){
                      $counter = 1;
                      while (list($id,$supplier_name,$mobile,$email,$address,$iden_name,$iden_phone,$status) = $supplier_list->fetch_row()) {
                        if($status == 1) {
                          // activate member
